...
function AuthModal({mode,onClose,onSwitch}:{mode:"signup"|"login",onClose:()=>void,onSwitch:()=>void}){
 const[name,setName]=useState(""); const[email,setEmail]=useState(""); const[password,setPassword]=useState(""); const[show,setShow]=useState(false); const[done,setDone]=useState(false);
 const submit=(e:React.FormEvent)=>{e.preventDefault(); if(mode==="signup"){localStorage.setItem("immo_rca_user",JSON.stringify({name,email}));} setDone(true);};
